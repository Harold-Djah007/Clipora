...
class AppState extends ChangeNotifier {
  ...
  Future<void> resolveAndDownload(
    ...
    status = 'Clipora turbo queue ready: ${urls.length} post${urls.length == 1 ? '' : 's'}…';
    ...
    try {
      for (var postIndex = 0; postIndex < urls.length; postIndex++) {
        ...
        status = 'Turbo resolving post ${postIndex + 1} of ${urls.length}…';
        await PlatformServices.updateDownloadService(message: status!);
        notifyListeners();
        try {
          debugPrint('[Clipora] resolving: $url');
          final source = await sourceLoader(url);
          debugPrint('[Clipora] resolver source bytes=${source.length}');
          final post = parser.parse(source, url);
          debugPrint('[Clipora] parsed ${post.media.length} media item(s) for ${post.postId}');
          final result = await downloadManager.downloadPost(
            post,
            settings,
            onProgress: (c, total) {
              status = 'Saving $c of $total media item${total == 1 ? '' : 's'} from post ${postIndex + 1}…';
      ...
      if (failed == 0) {
        status = 'Done: saved $mediaDone media item${mediaDone == 1 ? '' : 's'} to your Clipora folders';
