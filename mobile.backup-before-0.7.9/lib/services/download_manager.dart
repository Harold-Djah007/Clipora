...
class DownloadManager {
  ...
  Future<List<DownloadRecord>> downloadPost(
    ...
    for (var i = 0; i < post.media.length; i++) {
      ...
      try {
        ...
        debugPrint('[ThreadVault] download ${i + 1}/${post.media.length} ${item.kind.name} host=${uri?.host ?? 'unknown'}');
        ...
        debugPrint('[ThreadVault] saved ${item.kind.name}: $base.$ext');
        record = record.copyWith(status: DownloadStatus.completed);
      } catch (e, st) {
        debugPrint('[ThreadVault] media download failed: $e\n$st');
        try {
  ...
  bool _isMp4(List<int> b) {
    // Most MP4s expose the ftyp box within the first few bytes.
